...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (isset($_POST['send_email'])) {
        ...
        if ($student) {
            $to      = $student['email'];
            $headers = "From: SMS Admin <no-reply@school.local>\r\nContent-Type: text/plain; charset=UTF-8";
            @mail($to, $subject, $body, $headers);
            // Also create in-system notification
            addNotification($student_id, "You have a message from the administrator regarding: <strong>" . htmlspecialchars($subject) . "</strong>. Please check your email (" . htmlspecialchars($to) . ").");
            logAction($_SESSION['user_id'], 'Email Student', "Emailed student ID: $student_id — Subject: $subject");
